...
function update(elapsed:Float) {
    ...
    if (FlxG.keys.justPressed.TWO) curSpeed -= 0.025;
    if (FlxG.keys.justPressed.THREE) curSpeed = 1;
    if (FlxG.keys.justPressed.FOUR) curSpeed += 0.025;
...
function updateBotplay(elapsed:Float) {
    ...
    if (FlxG.keys.justPressed.EIGHT) curBotplay = !curBotplay;
...
function updateSpeed(speed:Float)
{
    ...
    for (strumLine in strumLines.members) {
        strumLine.vocals.pitch = vocals.pitch;
    }
}
